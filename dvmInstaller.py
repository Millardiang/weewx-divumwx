        ...
        try:
            ...
            if conf_file == None:
                ...
                if len(conf_files) == 1:
                    ...
                    if file_count > 1:
                        print("\nList of found dvmInstaller conf files to install that have existing weewx paths") 
                        ...
                        print("!!! NO VALID dvmInstaller CONFIG FILE. INSTALL ABORTED!!!")
                        sys.exit(1)
            print("dvmInstaller Config file " + conf_file + " was chosen.")
            try:
            ...
            if www_path != config_data['StdReport'].get('HTML_ROOT'):
                print("!!! WEEWX HTML ROOT  " + config_data['StdReport'].get('HTML_ROOT') + "  DOES NOT MATCH dvmInstaller www PATH.  " + www_path + "  INSTALL ABORTED!!!") 
                sys.exit(1)
            with open(services_file) as infile:
            ...
            serviceString = config_data['Engine']['Services'].get('data_services')
            if config_data['Engine']['Services'].get('data_services') == ',' or config_data['Engine']['Services'].get('data_services') == '""' or "user.divumwx.FilePile" not in serviceString:
               config_data['Engine']['Services']['data_services'] = 'user.divumwx.FilePile'
            elif "user.divumwx.FilePile" in serviceString:
                 config_data['Engine']['Services']['data_services'] = config_data['Engine']['Services'].get('data_services')
            elif "user.divumwx.FilePile" not in serviceString:
                 config_data['Engine']['Services']['data_services'] = config_data['Engine']['Services'].get('data_services') + ', user.divumwx.FilePile'
            config_data.write()
