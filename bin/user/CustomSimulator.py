...
def loader(config_dict, engine):
    ...
    stn = CustomSimulator(start_time=start_ts, resume_time=resume_ts, **config_dict[DRIVER_NAME])
...
class CustomSimulator(weewx.drivers.AbstractDevice):
    """Station simulator"""
    ...
    def __init__(self, **stn_dict):
        ...
        self.observations = {
            ...
            'rain'       : Rain(rain_start=12, rain_length=1, total_rain=0.1, loop_interval=self.loop_interval),
        	'stormRain'  : stormRain(Srain_start=12, Srain_length=12, Stotal_rain=12.55, Sloop_interval=self.loop_interval),
...
class stormRain:
    ...
    def __init__(self, Srain_start=0, Srain_length=1, Stotal_rain=0.1, Sloop_interval=2.5):
        """Initialize a stormRain simulator
        ...
        Args:
            ...
            loop_interval (float): Interval between LOOP packets.
        """
        Snpackets = 3600 * Srain_length / Sloop_interval
        Sn_rain_packets = Stotal_rain / stormRain.Sbucket_tip
        self.period = int(Snpackets / Sn_rain_packets)
        self.rain_start = 3600 * Srain_start
        self.rain_end = self.rain_start + 3600 * Srain_length
        self.packet_number = 0
    ...
    def value_at(self, time_ts):
        ...
        secs_since_midnight = time_tt.tm_hour * 3600 + time_tt.tm_min * 60.0 + time_tt.tm_sec
        if self.rain_start < secs_since_midnight <= self.rain_end:
            Samt = stormRain.Sbucket_tip if self.packet_number % self.period == 0 else 0.0
            self.packet_number += 1
        else:
            self.packet_number = 0
            Samt = 0				
...
if __name__ == "__main__":
    station = CustomSimulator(mode='simulator',loop_interval=2.0)
