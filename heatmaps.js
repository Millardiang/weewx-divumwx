...
(function(){
  ...
  // Card-visibility filtering (see cardSensorMap.js). Clean 1:1 mapping
  // for every category here -- unlike charts-d3.html's records-tab
  // categories, this file already separates tipping-bucket rain ('rain')
  // from piezo rain ('prain') into distinct categories, so there's no
  // OR-case or "no piezo data path" gap to work around.
  ...
  function applyTabVisibility(){
    ...
    // If the category we landed on (from the URL, or the 'temperature'
    // default) turned out to be hidden, fall through to the first
    // visible tab -- same reasoning as charts-d3.html's equivalent
    // fallback: otherwise the page shows a chart for a hidden category
    // with no way to tell from the tab bar which one it even is.
    if (currentTabHidden) {
