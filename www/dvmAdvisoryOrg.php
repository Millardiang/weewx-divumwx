...
///MetOffice
if ($alertlevel !== "none")
{$alertPhrase = '<spanelightning><alertadvisory><a alt="Alerts" title="Alerts" href= "' . $advisory . '" data-lity><img src="css/svg/' . $warnimage . '" width="100%" class="iconpos"></img></alertadvisory><alertpos><alertvalue>'.$uppercasealert.'<br> '.$alerttype.'</alertvalue></alertpos>
   </spanelightning></div></div></div>';
}
  
  //outlook
  else if ($alertlevel === "none")
  {$alertPhrase = '<outlook-panel><p>'.$outlookphrase.'</p></outlook-panel></div></div></div>';} 
