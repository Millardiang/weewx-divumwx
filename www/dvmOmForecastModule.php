...
$jsonfile="jsondata/openmeteo.txt";if(!file_exists($jsonfile)) {return;}
...
$forecasturl = file_get_contents("jsondata/openmeteo.txt");
if(filesize('jsondata/openmeteo.txt')<1){echo "".$offline. "";}
...
for ($k=0;$k<=2;$k++) 
{
     ...
     $Time[$k] = strtotime($parsed_json["hourly"]["time"][$k]);
     echo $forecastdayTempHigh = $parsed_forecastjson["daily"]["temperature_2m_max"][$k];
     echo $forecastdayTempLow = $parsed_forecastjson["daily_units"]["temperature_2m_min"][$k];
