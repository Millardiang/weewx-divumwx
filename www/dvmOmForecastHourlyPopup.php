...
<head>
  <meta charset="UTF-8">
  <title>Open-Meteo Hourly Forecast</title>
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
<!--<link href="css/popup.light.css?version=<?php echo filemtime('css/popup.light.css'); ?>" rel="stylesheet prefetch">//-->

<body>
<?php
$isoDate = date('Y-m-d\TH:00');
$forecastime = filemtime('jsondata/openmeteo.txt'); ?>
<?php include('forecastSelect.php');?> 
<main class="grid">
  <?php
$iconset = "icon2";
$jsonIcon = 'jsondata/lookupWmoIcons.json';
$jsonIcon = file_get_contents($jsonIcon);
$parsed_icon = json_decode($jsonIcon, true);
$forecastime = filemtime('jsondata/openmeteo.txt');
$json = 'jsondata/openmeteo.txt';
$json = file_get_contents($json);
$parsed_json = json_decode($json, true);
$timeIndex = array_search($isoDate, $parsed_json["hourly"]["time"])+1;
for ($k = $timeIndex;$k < $timeIndex + 12;$k++)
{
   $forecastWeatherCode[$k] = $parsed_json["hourly"]["weather_code"][$k];
    $forecastDayNight[$k] = $parsed_json["hourly"]["is_day"][$k];
   $forecastIconCode[$k] = $forecastWeatherCode[$k]."-".$forecastDayNight[$k];
    $forecastIcon[$k] = $parsed_icon["$forecastIconCode[$k]"]["icon2"];
    $timeString[$k] = strtotime($parsed_json["hourly"]["time"][$k]);
    $forecastTime[$k] = date("H:i", $timeString[$k]);
    $forecastTempHigh[$k] = $parsed_json["hourly"]["temperature_2m"][$k];
    $forecastWindGust[$k] = $parsed_json["hourly"]["wind_gusts_10m"][$k];
    $forecastWindSpeed[$k] = $parsed_json["hourly"]["wind_speed_10m"][$k];
    $forecastWinddirectionDeg[$k] = $parsed_json["hourly"]["wind_direction_10m"][$k];
    if ($forecastWinddirectionDeg[$k] <15 ) {$forecastWinddircardinal[$k]='North';}
				  elseif ($forecastWinddirectionDeg[$k] <45 ) {$forecastWinddircardinal[$k]='NNE';}
				  elseif ($forecastWinddirectionDeg[$k] <90 ) {$forecastWinddircardinal[$k]='ENE';}
				  elseif ($forecastWinddirectionDeg[$k] <110 ) {$forecastWinddircardinal[$k]='East';}
				  elseif ($forecastWinddirectionDeg[$k] <150 ) {$forecastWinddircardinal[$k]='SE';}
				  elseif ($forecastWinddirectionDeg[$k] <170 ) {$forecastWinddircardinal[$k]='SSE';}
				  elseif ($forecastWinddirectionDeg[$k] <190 ) {$forecastWinddircardinal[$k]='South';}
				  elseif ($forecastWinddirectionDeg[$k] <220 ) {$forecastWinddircardinal[$k]='SSW';}
				  elseif ($forecastWinddirectionDeg[$k] <250 ) {$forecastWinddircardinal[$k]='SW';}
				  elseif ($forecastWinddirectionDeg[$k] <270 ) {$forecastWinddircardinal[$k]='West';}
				  elseif ($forecastWinddirectionDeg[$k] <300 ) {$forecastWinddircardinal[$k]='NW';}
				  elseif ($forecastWinddirectionDeg[$k] <340 ) {$forecastWinddircardinal[$k]='NWN';}
				  elseif ($forecastWinddirectionDeg[$k] <360 ) {$forecastWinddircardinal[$k]='North';}
    $forecastprecipIntensity[$k] = $parsed_json["hourly"]["precipitation"][$k];
    $forecastPrecipProb[$k] = $parsed_json["hourly"]["precipitation_probability"][$k];
    $forecastUV[$k] = $parsed_json["hourly"]["uv_index"][$k];
    //$forecastsnow[$k] = $parsed_json['response'][0]['periods'][$k]['snowCM'];
    //$forecastsummary[$k] = $parsed_json['response'][0]['periods'][$k]['weather'];
   
    $forecastdesc[$k] = $parsed_icon["$forecastIconCode[$k]"]["description"];
    //$forecastheatindex[$k] = $parsed_json['response'][0]['periods'][$k]['avgFeelslikeC'];
    $forecasthumidity[$k] = $parsed_json["hourly"]["relative_humidity_2m"][$k];
