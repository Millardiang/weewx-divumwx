...
function pm25_to_aqi($pm25){
     if ($pm25 > 225.5 ) {
	  $aqi25 = map($pm25, 225.5, 350.5, 300, 400);
	} else if ($pm25 > 125.5 && $pm25 <= 225.5 ) {
	  $aqi25 = map($pm25, 125.5, 225.4, 200, 300);
	} else if ($pm25 > 55.5 && $pm25 <= 125.5 ) {
	  $aqi25 = map($pm25, 55.5, 125.4, 150, 200);
	} else if ($pm25 > 35.5 && $pm25 <= 55.5 ) {
	  $aqi25 = map($pm25, 35.5, 55.4, 100, 150);
	} else if ($pm25 > 9 && $pm25 <= 35.5 ) {
	  $aqi25 = map($pm25, 9.1, 35.4, 50, 100);
	} else if ($pm25 > 0 && $pm25 <= 9 ) {
	  $aqi25 = map($pm25, 0, 9, 0, 50);
	}
	return $aqi25;
}
